...
public class RepoBuilder {
    ...
    public static Single<Integer> getRepoContributorsCount(final RemoteAPIService service, final String repoFullName) {
        HashMap<String, String> queryMap = new HashMap<>();

        return service.queryRepoContributors(repoFullName, queryMap)
                .flatMap(new Function<Response<List<UserEntry>>, SingleSource<Integer>>() {
                    @Override
                    public SingleSource<Integer> apply(Response<List<UserEntry>> listResponse) throws Exception {
                        HashMap<String, String> lastPageQuery = new HashMap<>();
                        if (listResponse.headers().get("Link") != null) {
                            lastPageQuery.put("page", Utils.getLastPageFromLinkHeader(listResponse.headers().get("Link")));
                        }
                                ...
                                .zipWith(service.queryRepoContributors(repoFullName, lastPageQuery),
                                        new BiFunction<Response<List<UserEntry>>, Response<List<UserEntry>>, Integer>() {
                                            ...
                                            public Integer apply(Response<List<UserEntry>> listResponse,
                                                ...
                                                if (listResponse.headers().get("Link") != null) {
                                                    String pages = Utils.getLastPageFromLinkHeader(listResponse.headers().get("Link"));
    ...
    public static Single<Integer> getRepoCommitsCount(final RemoteAPIService service, final String repoFullName) {
        HashMap<String, String> queryMap = new HashMap<>();

        return service.queryRepoCommits(repoFullName, queryMap)
                .flatMap(new Function<Response<List<CommitEntry>>, SingleSource<Integer>>() {
                    @Override
                    public SingleSource<Integer> apply(Response<List<CommitEntry>> listResponse) throws Exception {
                        HashMap<String, String> lastPageQuery = new HashMap<>();
                        if (listResponse.headers().get("Link") != null) {
                            lastPageQuery.put("page", Utils.getLastPageFromLinkHeader(listResponse.headers().get("Link")));
                        }
    ...
    public static Single<Integer> getRepoReleasesCount(final RemoteAPIService service, final String repoFullName) {
        HashMap<String, String> queryMap = new HashMap<>();

        return service.queryRepoReleases(repoFullName, queryMap)
                .flatMap(new Function<Response<List<ReleaseEntry>>, SingleSource<Integer>>() {
                    @Override
                    public SingleSource<Integer> apply(Response<List<ReleaseEntry>> listResponse) throws Exception {
                        HashMap<String, String> lastPageQuery = new HashMap<>();
                        if (listResponse.headers().get("Link") != null) {
                            lastPageQuery.put("page", Utils.getLastPageFromLinkHeader(listResponse.headers().get("Link")));
                        }
    ...
    public static Single<Integer> getRepoBranchesCount(final RemoteAPIService service, final String repoFullName) {
        HashMap<String, String> queryMap = new HashMap<>();

        return service.queryRepoBranches(repoFullName, queryMap)
                .flatMap(new Function<Response<List<BranchEntry>>, SingleSource<Integer>>() {
                    @Override
                    public SingleSource<Integer> apply(Response<List<BranchEntry>> listResponse) throws Exception {
                        HashMap<String, String> lastPageQuery = new HashMap<>();
                        if (listResponse.headers().get("Link") != null) {
                            lastPageQuery.put("page", Utils.getLastPageFromLinkHeader(listResponse.headers().get("Link")));
                        }
